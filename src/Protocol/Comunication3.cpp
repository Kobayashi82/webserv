...
/*   Updated: 2024/09/19 21:14:15 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	#pragma region PROCESS
		...
		#pragma region Response
			...
			void Comunication::process_response(EventInfo * event) {
				...
				if (event->no_cache == false) {
					...
					if (fcache) {
						std::string response = 
							...
							"Content-Length: " + Utils::ltos(fcache->size) + "\r\n"
							"Connection: close\r\n"
							"\r\n";

						event->file_info = 0;
						event->file_read = 0;
						event->file_size = response.size() + fcache->size;
				...
				size_t chunk = CHUNK_SIZE;
				if (filesize > 0) chunk = std::min(event_data.file_size, CHUNK_SIZE);
				...
				event->file_info = 0;
				event->file_read = 0;
				event->file_size = response.size() + filesize;
				event->write_buffer.clear();
				event->write_buffer.insert(event->write_buffer.end(), response.begin(), response.end());
				if (Epoll::add(event_data.fd, true, false) == -1 || Epoll::set(event->fd, true, true) == -1) {
					event->file_size = 0;
					event->write_buffer.clear();
