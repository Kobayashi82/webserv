...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($lines as $line) {
        ...
        // Convertir tanto el usuario almacenado como el ingresado a minúsculas
        $storedUserLower = strtolower($storedUser);
        $usernameLower = strtolower($username);

        // Comprobar si el usuario y la contraseña coinciden
        if ($storedUserLower == $usernameLower && $storedPass == $password) {
            ...
            if (isset($postData['remember_me']) && $postData['remember_me'] == '1') {
                ...
                if (isset($_COOKIE['user_session_cookie'])) {
                    setcookie('user_session_cookie', '', time() - 3600, "/"); // Eliminar cookie
                }
            }
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = '/contact.html'; // Redirige a contact.html en caso de éxito
        } else {
